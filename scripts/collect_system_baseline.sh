...
record_shell "08_build_tools" "cmake --version && gcc --version && g++ --version && if command -v colcon >/dev/null 2>&1; then command -v colcon; colcon --help | head -n 40; dpkg-query --show python3-colcon-core 2>/dev/null || true; else echo 'colcon: missing'; exit 1; fi"
